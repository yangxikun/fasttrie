...
func TestTreeAddAndGet(t *testing.T) {
	...
	checkRequests(t, tree, testRequests{
		...
		{"/hello/test1", false, "/hello/{name}", map[string]string{"name": "test1"}},
...
func TestTreeTrailingSlashRedirect(t *testing.T) {
	...
	for _, route := range tsrRoutes {
		value, tsr := tree.Get(route, nil)
		if value != nil {
			t.Fatalf("non-nil value for TSR route '%s", route)
		} else if !tsr {
	...
	for _, route := range noTsrRoutes {
		value, tsr := tree.Get(route, nil)
		if value != nil {
			t.Fatalf("non-nil value for No-TSR route '%s", route)
		} else if tsr {
...
func TestTreeRootTrailingSlashRedirect(t *testing.T) {
	...
	value, tsr := tree.Get("/", nil)
	if value != nil {
		t.Fatalf("non-nil value")
	} else if tsr {
...
func TestTreeWildcardConflictEx(t *testing.T) {
	...
	conflicts := []struct {
		...
		},
		{
			route:       "/con{tact}",
			wantErr:     true,
			wantErrText: "a value is already registered for path '/con{tact}'",
		},
		{
			route:       "/con{something}",
			wantErr:     true,
		...
		},
		{
			route:       "/who/are/{you:*}",
			wantErr:     true,
			wantErrText: "a wildcard value is already registered for path '/who/are/{you:*}'",
		},
		{
			route:       "/who/are/{me:*}",
			wantErr:     true,
		...
		},
		{
			route:       "/who/foo/hello",
			wantErr:     true,
			wantErrText: "a value is already registered for path '/who/foo/hello'",
		},
		{
			route:       "/{static:*}",
			wantErr:     true,
	...
	for _, conflict := range conflicts {
		...
		if err != nil && conflict.wantErrText != fmt.Sprint(err) {
			t.Errorf("Invalid conflict error text (%v), want: %s", err, conflict.wantErrText)
		}
	}
}
